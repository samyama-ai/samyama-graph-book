...
Samyama, Neo4j runs the subset in \textbf{7.8~s} vs.\ Samyama's
\textbf{34.4~s} (4.4$\times$ faster overall) on this 8M-node graph.
Per-query, the comparison is more balanced than the totals suggest:
Samyama wins 20/55, Neo4j wins 29/55, and 6 are within 1.5$\times$.
The 4.4$\times$ total-time gap is concentrated in four outlier
adj-agg queries (CT08/10/15/20: 4--9~s each) where Samyama's
\texttt{count(DISTINCT)} and 2-hop \texttt{WITH} planning still
trail Neo4j's mature operators.  Outside those four, Samyama is
within 1.5$\times$ on most queries and faster on the trivial-lookup
shapes that dominate the long tail.
