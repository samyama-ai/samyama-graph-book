...
On raw latency, however, Neo4j won 47/55 queries; Samyama won 2;
6 were within 1.5$\times$.  Neo4j's mature query optimizer and
B-tree indexes make it highly effective at this scale.

The comparison does not exercise the system's two main design
contributions: (i)~\emph{multi-tenant federation} requires Neo4j
Enterprise + Fabric, which is out of scope; (ii)~\emph{adjacency-aware
aggregation on billion-edge graphs} targets PubMed's 1.04B~edges,
100$\times$ larger than the merged subset.  At the 8M-node scale
Neo4j Community is the better-engineered choice; the case for our
system rests on the federation architecture and headroom at
billion-edge scale.
