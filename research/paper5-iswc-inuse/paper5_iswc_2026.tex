...
Late materialization reduces peak memory allocation by ${\sim}60\%$
on 100K-node benchmarks compared to eager materialization (measured
by toggling the resolve strategy in the same codebase).  The cost is
an additional indirection per property access in \texttt{WHERE}
filters---negligible for selective queries but measurable on
full-scan aggregations where every node's properties are accessed
regardless.  For \texttt{LIMIT}~$k$ queries the benefit is
unambiguous: only $k$ nodes are ever materialized.
