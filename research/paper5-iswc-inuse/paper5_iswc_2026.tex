...
Samyama, Neo4j runs the subset in \textbf{9.5~s} vs.\ Samyama's
\textbf{18.7~s} (2.0$\times$ faster overall) on this 8M-node graph.
...
($\S$\ref{sec:adj-agg}) the score is even (Samyama 7, Neo4j 7,
3 tied).  The remaining gap is concentrated in a single 2-hop
query (CT20: chain anchored on a \texttt{WITH}-bound aggregate)
that falls outside the current optimized shape; Samyama issues
the generic plan there, and a direct extension of the same
operator family would close it.
