...
The Samyama engine source code, benchmark queries, expected-rows
oracle, KG snapshots, and MCP tool definitions are released under
open-source licenses.  The 500 benchmark queries are available as
a YAML file in the project repository, paired with the oracle that
records expected row counts and documented data-dependent gaps.
Reproducing the full evaluation requires an instance with at least
512\,GB RAM (we used AWS r7i.16xlarge); the seven-KG subset (PubMed
excluded) reproduces on a 16\,GB workstation.  Snapshot files
(\texttt{.sgsnap} format) and a runner script
(\texttt{unified\_benchmark.rs}) are included to enable a
single-command rerun.  The 40~BiomedQA questions and 39~MCP tool
definitions are likewise public.  Repository:
\url{https://github.com/samyama-ai}.

%% ================================================================
\section{Limitations}\label{sec:limitations}

Several limitations should be noted.
(1)~The benchmark queries were authored by the system developers,
creating a risk of co-adaptation between queries and engine
capabilities.  We mitigate this by publishing the full query set
and oracle, but independent validation by external users remains
future work.
(2)~Bridge-property federation assumes that shared identifiers are
consistent across KGs; when they are not (e.g., drug-name casing),
queries silently return incomplete results rather than errors.
(3)~The system currently runs as a single-node deployment; the
512\,GB memory requirement for the full graph limits adoption.
(4)~The BiomedQA evaluation uses 40~questions---sufficient to
demonstrate the MCP vs.\ text-to-Cypher gap, but too small for
robust statistical conclusions.
(5)~The OMOP KG uses synthetic patient data (Synthea), and results
on comorbidity and cohort queries may not transfer to real clinical
deployments.

\section{Conclusion and Future Work}\label{sec:conclusion}

We reported on the deployment and evaluation of a federated
biomedical knowledge graph system spanning ten KGs (137M~nodes,
1.22B~edges).  The main empirical findings are: (1)~bridge-property
federation scales to billion-edge graphs given exact per-triple
statistics, but is limited by identifier coverage across KGs;
(2)~adjacency-aware aggregation eliminates a common class of
timeout queries on high-degree graphs; and (3)~domain-specific MCP
tools outperform generic text-to-Cypher by encoding schema knowledge,
at the cost of manual authoring effort.

Future work includes: (1)~compressed storage to reduce the memory
footprint below 128\,GB, enabling deployment on commodity hardware;
(2)~UMLS Metathesaurus as a concept-resolution spine to improve
bridge-join coverage; and (3)~a larger-scale MCP evaluation with
external users and a broader question set.
