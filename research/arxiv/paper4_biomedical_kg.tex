...
All data sources are open-license (CC~BY~4.0, CC0, OBO). Snapshots, ETL code, and MCP configurations are publicly available. The combined federated graph (7.83M nodes, 27.9M edges) loads in under 3 minutes on commodity hardware (Mac Mini M4, 16GB RAM).
...
\midrule
\multicolumn{2}{l}{\textbf{Total nodes}} & \textbf{7,711,965} \\
\midrule
\multicolumn{2}{l}{\textbf{Total edges}} & \textbf{27,069,085} \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
Pathways KG & 118,686 & 834,785 & 9 MB & $<$ 5 s \\
Clinical Trials KG & 7,711,965 & 27,069,085 & 711 MB & $\sim$90 s \\
\midrule
\textbf{Combined} & \textbf{7,830,651} & \textbf{27,903,870} & \textbf{720 MB} & \textbf{$\sim$95 s} \\
\bottomrule
\end{tabular}
\end{table}

Snapshot import uses gzip-compressed JSON-lines format. The Pathways KG loads in under 5 seconds; the Clinical Trials KG, with its 7.7M nodes, loads in approximately 90 seconds. Both load into the same tenant for federation.
...
\begin{table}[H]
\centering
\caption{Query latency on Mac Mini M4 (median of 5 runs).}
\label{tab:queries}
\small
...
\midrule
Label distribution & Pathways & 5 labels & $<$1 ms \\
Top pathways by protein count & Pathways & 10 rows & 85 ms \\
PPI hub proteins & Pathways & 10 rows & 420 ms \\
TP53 2-hop PPI neighborhood & Pathways & 15 rows & 1.2 s \\
Pathway crosstalk ($\geq$100 shared) & Pathways & 10 rows & 3.8 s \\
\midrule
Phase 3 breast cancer trials & Clinical & varies & 120 ms \\
Trials by sponsor class & Clinical & 7 rows & 45 ms \\
\midrule
\textbf{Federated: Drug $\rightarrow$ Pathway} & \textbf{Combined} & 15 rows & 2.5 s \\
\textbf{Federated: Disease $\rightarrow$ Pathway} & \textbf{Combined} & 10 rows & 1.8 s \\
\bottomrule
\end{tabular}
\end{table}

Federated queries execute in 1--4 seconds, which is acceptable for interactive exploration. The property-based join (\texttt{WHERE p1.uniprot\_id = p2.uniprot\_id}) is the primary cost; creating an index on \texttt{Protein.uniprot\_id} would reduce this to sub-second.

\subsection{Federation Correctness}

To validate that cross-KG joins produce meaningful results, we verified three properties:

\begin{enumerate}[leftmargin=*]
  \item \textbf{Join overlap}: Proteins with \texttt{uniprot\_id} appear in both snapshots (verified by counting shared IDs).
  \item \textbf{Transitivity}: A drug that targets a protein in the Clinical Trials KG, where that protein participates in a pathway in the Pathways KG, correctly produces the pathway in the federated query result.
  \item \textbf{Completeness}: The federated query returns a superset of pathways obtainable by manually cross-referencing DrugBank target data with Reactome pathway membership.
\end{enumerate}
...
We presented two open-source biomedical knowledge graphs---Pathways KG and Clinical Trials KG---built on Samyama Graph Database. Together they contain 7.83 million nodes and 27.9 million edges from 10 public data sources. We demonstrated that loading both snapshots into a single graph tenant enables cross-KG federated queries that bridge molecular biology and translational medicine. We introduced schema-driven MCP server generation for LLM agent access to graph queries.

All code, snapshots, ETL pipelines, and MCP configurations are open-source under Apache License 2.0. Researchers can reproduce the full pipeline---from data download to federated queries---in under 10 minutes on commodity hardware.
