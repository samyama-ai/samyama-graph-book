...
\author{
  Madhulatha Mandarapu\textsuperscript{1}\thanks{madhulatha@samyama.ai, ORCID: \url{https://orcid.org/0009-0005-2837-6725}} \and
  Sandeep Kunkunuru\textsuperscript{1}\thanks{sandeep@samyama.ai, ORCID: \url{https://orcid.org/0000-0002-8886-1846}}
}
\date{March 2026 \quad|\quad v0.6.0}
...
The engine implements \textbf{30 physical operators} organized across scan, traversal, filter, join, aggregation, sort, write, index, and specialized categories. Key v0.6.0 additions include:
...
The Python SDK (v0.6.0) exposes direct algorithm methods (\texttt{page\_rank}, \texttt{wcc}, \texttt{scc}, \texttt{bfs}, \texttt{dijkstra}, \texttt{pca}, \texttt{triangle\_count}) in embedded mode, enabling data scientists to run graph algorithms directly from Python without writing Cypher.
...
\begin{itemize}[leftmargin=*]
  \item \textbf{LDBC SNB Interactive}: 21 queries (IS1--IS7, IC1--IC14) + 8 update operations on the Social Network Benchmark SF1 dataset. Tests OLTP-style point lookups and multi-hop traversals.
  \item \textbf{LDBC SNB Business Intelligence}: 20 complex analytical queries (BI-1 to BI-20) testing OLAP-style aggregation over the social network graph.
  \item \textbf{LDBC FinBench}: 40+ queries (CR1--CR12, SR1--SR6, RW1--RW3, W1--W19) modeling financial transaction networks with accounts, transfers, loans, and fraud detection patterns.
\end{itemize}
