...
We replace IBM's flat document stores with a knowledge graph (781 nodes, 955 edges, 16 relationship types) built on the Samyama graph database, and evaluate three architectures of increasing LLM involvement: (1) deterministic graph handlers with no LLM, achieving 99\% (137/139); (2) LLM-generated Cypher queries against the graph, achieving 83\% (115/139); and (3) IBM's original LLM-over-flat-documents approach at 65\% (91/139).
...
These results suggest that for structured operational domains, the data model---not the LLM---is the primary bottleneck, and that knowledge graphs can serve as a missing integration layer between raw industrial data and LLM-based reasoning.
\end{abstract}
...
IBM's AssetOpsBench~\citep{ibm2024assetopsbench} provides the first systematic evaluation of such agents, benchmarking GPT-4 across 139 industrial maintenance scenarios spanning five agent types: IoT telemetry, failure mode and symptom reasoning (FMSR), work order management, time-series foundation model queries, and multi-agent coordination. Their architecture follows the now-standard tool-augmented LLM pattern~\citep{schick2023toolformer,yao2023react}: the LLM parses user intent, selects the appropriate tool, formulates arguments, retrieves data from flat document stores (CouchDB~\citep{anderson2010couchdb}, YAML, CSV), and synthesizes an answer. The LLM does \emph{everything}---and achieves 65\%.

We observe that many of the LLM's failures are not failures of reasoning but failures of \emph{data access}: hallucinated equipment identifiers, miscounted events across documents, inability to traverse equipment dependencies, and fabricated sensor readings. These are symptoms of asking an LLM to perform \emph{data operations}---counting, joining, traversing---that structured query engines handle deterministically.

This observation motivates our central hypothesis: \textbf{the bottleneck in LLM-based industrial operations is the data model, not the LLM.} Replacing flat document stores with a knowledge graph~\citep{hogan2021knowledge} should improve accuracy at every level of LLM involvement.

\paragraph{Contributions.}
\begin{enumerate}[leftmargin=*]
  \item \textbf{Three-tier evaluation} of the same 139 scenarios under three architectures: deterministic graph handlers (99\%), LLM-generated Cypher over the graph (83\%), and IBM's LLM-over-flat-documents baseline (65\%). This isolates the data model as the key variable (\S\ref{sec:evaluation}).
  ...
  \item \textbf{Knowledge graph construction}: a complete ETL pipeline transforming IBM's flat data sources into an 11-label, 16-edge-type graph with 384-dimensional failure mode embeddings and equipment dependency topology (\S\ref{sec:kg-construction}).
  \item \textbf{40 new graph-native scenarios} covering multi-hop dependency analysis, vector similarity search, PageRank criticality, cascade analysis, maintenance optimization, and temporal patterns---capabilities structurally impossible with flat document stores (\S\ref{sec:new-scenarios}).
...
AssetOpsBench~\citep{ibm2024assetopsbench} benchmarks \emph{LLM agents}, not data systems. The evaluation measures whether an LLM can autonomously navigate a set of tools to answer industrial maintenance questions. The benchmark comprises 139 scenarios across five agent types (Table~\ref{tab:ibm-scenarios}).

\begin{table}[t]
  \centering
  \caption{AssetOpsBench scenario distribution.}
  \label{tab:ibm-scenarios}
  \begin{tabular}{@{}llr@{}}
    \toprule
    \textbf{Agent Type} & \textbf{Domain} & \textbf{Count} \\
    \midrule
    IoT          & Site/asset/sensor hierarchy queries  & 20 \\
    FMSR         & Failure mode and symptom reasoning   & 40 \\
    WO           & Work order and event management      & 36 \\
    TSFM         & Time-series foundation model queries & 23 \\
    Multi-Agent  & Cross-domain coordination            & 20 \\
    \midrule
...
IBM's architecture follows a ReAct-style~\citep{yao2023react} loop: the user poses a natural language question; the LLM parses intent, selects which agent to invoke, formulates tool arguments, retrieves raw data from CouchDB/YAML/CSV, interprets the results, and synthesizes an answer. The data sources are flat: JSON documents in CouchDB, YAML configuration files for failure modes, and CSV exports for events and work orders. There is no graph, no vector index, and no relationship schema.

IBM reports that GPT-4 achieves approximately 65\% (91/139) on these scenarios. The failures cluster around tasks requiring counting across documents, correlating data from multiple sources, and traversing implicit equipment relationships---precisely the operations that LLMs are known to perform poorly.
...
We transform IBM's flat data sources into a typed knowledge graph using an 8-step ETL pipeline. The resulting graph contains 781 nodes across 11 labels, connected by 955 edges across 16 relationship types (Table~\ref{tab:schema}).

\begin{table}[t]
  \centering
  \caption{Knowledge graph schema: 11 node labels, 16 edge types.}
  \label{tab:schema}
  \begin{tabular}{@{}lp{9cm}@{}}
    \toprule
    \textbf{Node Labels} & Site, Location, Equipment, Sensor, FailureMode, WorkOrder, SparePart, Supplier, SensorReading, Anomaly, Event \\
    \midrule
...
The ETL pipeline processes four IBM data sources into graph structure:
...
\subsection{Architecture A: LLM Does Everything (IBM Baseline)}

\begin{quote}
Question $\to$ LLM parses intent $\to$ LLM selects tool $\to$ LLM formulates arguments $\to$ Tool queries flat store $\to$ LLM interprets raw results $\to$ LLM synthesizes answer
\end{quote}

The LLM handles five distinct subtasks: intent parsing, tool selection, argument crafting, data interpretation, and answer synthesis. The data sources are flat document stores with no typed schema or relationship structure. This is the architecture IBM benchmarks; GPT-4 achieves 65\%.
...
The key insight connecting Architectures A and B is what we term \textbf{inverted LLM usage}. Both use an LLM, but they ask fundamentally different questions:

\begin{itemize}[leftmargin=*]
  \item \textbf{IBM} asks: ``LLM, answer this question from this data.'' The LLM must count across documents, correlate fields from different sources, and traverse implicit relationships---tasks where LLMs consistently fail.
  \item \textbf{NLQ} asks: ``LLM, given this schema, write me a Cypher query.'' The LLM performs code generation from a structured specification---a task where LLMs consistently excel.
\end{itemize}
...
\subsection{IBM's 139 Scenarios: Three-Tier Results}

Table~\ref{tab:main-results} presents the primary comparison across all three architectures on IBM's original 139 scenarios.

\begin{table}[t]
  \centering
  \caption{Three-tier performance on IBM's 139 AssetOpsBench scenarios.}
  \label{tab:main-results}
  \begin{tabular}{@{}lllrr@{}}
    \toprule
    \textbf{Architecture} & \textbf{LLM Role} & \textbf{Data Layer} & \textbf{Pass Rate} & \textbf{Avg Latency} \\
    \midrule
    A: IBM baseline     & Does everything     & Flat (CouchDB/YAML/CSV) & 91/139 (65\%)  & not reported \\
...
\paragraph{Model caveat.} IBM used GPT-4; our NLQ benchmark used GPT-4o (a more capable model released later). The +18 percentage point gap between NLQ (83\%) and IBM (65\%) is therefore an \emph{upper bound} on the graph's contribution---part of the improvement may stem from the stronger LLM. A GPT-4 NLQ run is in progress to isolate the graph's effect.
...
\paragraph{Deterministic failures.} Only 2 of 139 scenarios fail: both are work order bundling edge cases (IDs 411, 424) where IBM's expected answer groups work orders into specific bundle sizes. Our date-window clustering produces different groupings. These are response-format mismatches, not knowledge gaps.
...
We designed 40 additional scenarios that test capabilities beyond IBM's scope---queries that require graph structure, vector search, or graph algorithms and are structurally impossible with flat document stores (Table~\ref{tab:custom-categories}).
...
IBM's GPT-4 agents fail not because GPT-4 lacks reasoning ability, but because flat document stores make certain queries structurally hard or impossible:

\begin{itemize}[leftmargin=*]
  \item \textbf{Counting across documents}: ``How many events for CWC04009 in 2019?'' requires the LLM to scan multiple JSON documents, parse dates, and aggregate---error-prone at scale. In the graph: \texttt{MATCH (e:Event) WHERE e.equipment\_id = 'CWC04009' RETURN count(e)} executes deterministically.

  \item \textbf{Relationship traversal}: ``Which sensors monitor overheating?'' is one edge hop in the graph: \texttt{(s:Sensor)-[:MONITORS]->(fm:FailureMode)}. In flat stores, the LLM must correlate YAML failure definitions with CouchDB sensor metadata---a cross-source join with no explicit link.

  \item \textbf{Impossible operations}: Multi-hop cascade analysis, vector similarity search, PageRank criticality, and Pareto-optimal scheduling have no equivalent on flat document stores regardless of LLM capability.
\end{itemize}
...
\begin{enumerate}[leftmargin=*]
  \item \textbf{Deterministic vs.\ autonomous}: Architecture C's 99\% result compares a pre-coded solution against an autonomous agent. We wrote the answers; IBM benchmarks whether GPT-4 can figure them out independently. These are fundamentally different tasks.
  \item \textbf{Model mismatch}: IBM used GPT-4; our NLQ runs used GPT-4o. The +18pp gap is an upper bound on the graph's contribution. A same-model comparison is in progress.
  ...
  \item \textbf{Custom scenarios}: Our 40 new scenarios are designed to showcase graph-native capabilities. They are complementary to, not replacements for, IBM's benchmark.
\end{enumerate}
...
\begin{enumerate}[leftmargin=*]
  \item \textbf{Data Ingestion} (software engineering): Raw sensor telemetry, CMMS exports, work order records, and equipment registries are loaded into the database via deterministic ETL pipelines. For structured data (90\%+ of industrial data), this is pure software engineering: \texttt{pandas.read\_csv()} $\to$ Cypher \texttt{CREATE} statements. IBM's pipeline is similarly deterministic---their \texttt{couchdb\_setup.sh} bulk-inserts JSON documents; their EAMLite uses an SQLModel ORM. No LLM is needed for structured ingestion.
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}lp{3.8cm}p{4.2cm}@{}}
    \toprule
    \textbf{Dimension} & \textbf{IBM (LLM + flat docs)} & \textbf{Samyama (graph $\pm$ LLM)} \\
    \midrule
...
\paragraph{LLM agents for tool use.} The ReAct framework~\citep{yao2023react} and Toolformer~\citep{schick2023toolformer} established the pattern of LLMs selecting and invoking external tools. AssetOpsBench~\citep{ibm2024assetopsbench} applies this pattern to industrial asset operations via the Model Context Protocol~\citep{mcp2024}. Our work complements this line of research by showing that the data layer behind the tools matters as much as the agent's reasoning capability.
...
We have shown that replacing flat document stores with a knowledge graph improves LLM-based industrial operations at every level of LLM involvement: from 65\% (LLM over flat docs) to 83\% (LLM generates queries over graph) to 99\% (deterministic graph handlers). The data model---not the LLM---is the primary bottleneck.
...
\paragraph{Contributions.} (1) A three-tier evaluation isolating the data model as the key variable on IBM's 139-scenario benchmark. (2) The inverted LLM usage pattern, demonstrating +18pp improvement by constraining the LLM to query generation. (3) A complete ETL pipeline and knowledge graph schema for industrial asset operations. (4) 40 new graph-native scenarios testing multi-hop, vector, and algorithmic capabilities. (5) An honest analysis of limitations including the model mismatch caveat and clean-data assumption.

\paragraph{Future work.} A same-model (GPT-4) NLQ comparison to isolate the graph's contribution from the model improvement; evaluation on larger industrial environments (10K+ assets, multi-site); integration of LLM-assisted data preparation for unstructured maintenance logs; and a hybrid architecture that uses deterministic handlers for known patterns and NLQ for novel queries.

\paragraph{Availability.} The benchmark code, ETL pipeline, and evaluation framework are available at \url{https://github.com/samyama-ai/assetops-kg}. The Samyama graph database is at \url{https://github.com/samyama-ai/samyama-graph}. The 40 new scenarios have been submitted to IBM's AssetOpsBench repository.
