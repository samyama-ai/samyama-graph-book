...
\date{%
  ...
  April 2026 \quad|\quad v0.7.0 (v3)
}
...
Our evaluation on commodity hardware demonstrates: ingestion at 255K nodes/s (CPU) and 412K nodes/s (GPU-accelerated); 115K Cypher queries/sec at 1M nodes; 4.0--4.7$\times$ latency reduction from late materialization; 8.2$\times$ GPU PageRank speedup; and 100\% LDBC Graphalytics validation (28/28 tests). \textbf{New in v3:} We demonstrate billion-edge scale by loading 74.3 million nodes and 1.07 billion edges across four biomedical knowledge graphs on a single AWS r6a.8xlarge instance for \$2.50 in spot compute. A 6-KG federation spanning molecular biology to population health (8 open data sources, 305K public health nodes) achieves 136/140 benchmark queries with sub-second latency. Worst-case optimal (WCO) joins via LeapFrog TrieJoin and Rayon-parallelized graph algorithms further extend the engine's capabilities.
\end{abstract}
...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{In-database metaheuristic optimization}: 22 solvers accessible directly via Cypher procedures, eliminating ETL overhead (\S\ref{sec:optimization}).
  \item \textbf{Agentic Enrichment (GAK)}: Autonomous graph expansion using LLM tool-calling (\S\ref{sec:agentic}).
  \item \textbf{Billion-edge knowledge graph federation} (v0.7.0): 74.3M nodes and 1.07B edges from 4 biomedical sources loaded on commodity hardware for \$2.50; 6-KG federation spanning molecular biology to population health with 136/140 benchmark queries passing (\S\ref{sec:billion-edge}, \S\ref{sec:public-health}).
\end{enumerate}
...
\begin{enumerate}[leftmargin=*,resume]
  ...
  \item \textbf{Cost-based query planner}: Plan enumeration, predicate pushdown, plan cache, ExpandInto, and join reordering (\S\ref{sec:query-engine}).
  \item \textbf{Worst-case optimal joins}: LeapFrog TrieJoin~\citep{veldhuizen2014triejoin} for cyclic patterns (v0.7.0).
  \item \textbf{Cross-platform GPU acceleration}: wgpu-based compute shaders for graph algorithms and PCA.
  \item \textbf{SDK ecosystem}: Rust, Python (PyO3), TypeScript SDKs with embedded and remote access.
  \item \textbf{Industry validation}: 100\% LDBC Graphalytics~\citep{iosup2016ldbc} (28/28), 100\% FinBench (40/40).
\end{enumerate}

This paper evaluates the four primary contributions: late materialization (\S\ref{sec:late-mat-eval}), in-database optimization (\S\ref{sec:opt-eval}), agentic enrichment (\S\ref{sec:gak-eval}), and billion-edge federation (\S\ref{sec:billion-edge}--\S\ref{sec:public-health}).
...
The engine implements \textbf{35 physical operators} organized across scan, traversal, filter, join, aggregation, sort, write, index, and specialized categories. Key additions include:
...
\paragraph{Worst-Case Optimal Joins (v0.7.0).}
For cyclic patterns (triangles, cliques), Samyama implements a \textbf{LeapFrog TrieJoin}~\citep{veldhuizen2014triejoin} operator. The planner detects triangle patterns in MATCH clauses and routes them to \texttt{LeapfrogJoinOperator}, which intersects sorted adjacency lists using seek-based merge. This avoids the quadratic intermediate blowup of traditional binary hash joins on cyclic patterns. The sorted adjacency invariant (maintained since v0.6.0) enables efficient seek operations.

\paragraph{Parallel Graph Algorithms (v0.7.0).}
The graph algorithm library now uses \textbf{Rayon} for data-parallel execution. PageRank, LCC, CDLP, and triangle counting distribute work across CPU cores via parallel iterators. On the Mac Mini M4 (10 cores), parallel PageRank on 1M nodes achieves ${\sim}4\times$ speedup over the sequential version.
...
\subsection{Billion-Edge Biomedical Knowledge Graph (v0.7.0)}
\label{sec:billion-edge}

To evaluate scale beyond LDBC synthetic datasets, we loaded a real-world biomedical knowledge graph federation from four open sources:

\begin{table}[H]
  \centering
  \caption{Biomedical trifecta dataset (real-world data).}
  \label{tab:trifecta}
  \begin{tabular}{@{}lrrrl@{}}
    \toprule
    \textbf{KG} & \textbf{Nodes} & \textbf{Edges} & \textbf{Size} & \textbf{Source} \\
    \midrule
    PubMed/MEDLINE   & 66.2M  & 1.04B  & 41 GB & NLM \\
    Clinical Trials  & 7.8M   & 27M    & 711 MB & AACT \\
    Pathways         & 119K   & 835K   & 9.1 MB & Reactome+STRING+GO \\
    Drug Interactions& 245K   & 388K   & 8.1 MB & DrugBank+4 sources \\
    \midrule
    \textbf{Total}   & \textbf{74.3M} & \textbf{1.07B} & & \\
    \bottomrule
  \end{tabular}
\end{table}

\paragraph{Loading.} The PubMed graph was loaded in \textbf{3 hours 46 minutes} on a single AWS r6a.8xlarge instance (32 vCPU, 256\,GB RAM) at a total spot cost of \textbf{\$2.50}. Two-phase bulk loading (node stubs at 777 bytes/node + edge stubs at 52 bytes/edge) achieves 13.6$\times$ memory reduction compared to full \texttt{create\_node()} calls. Mid-phase adjacency compaction every 50M edges prevents unbounded write-buffer growth.

\paragraph{Querying.} 100 benchmark queries span point lookups, 1-hop traversals, aggregations, and cross-KG federation. \textbf{96 of 100 queries return data}; 3 return empty results due to data gaps in specific snapshots; 1 times out on a cartesian-product query pattern. Latencies range from 1.3\,s (point lookup on 66M articles) to 19.4\,s (cross-KG aggregation spanning all 4 KGs).

\subsection{Public Health Knowledge Graph Federation (v0.7.0)}
\label{sec:public-health}

To demonstrate cross-domain federation beyond biomedicine, we built three public health knowledge graphs:

\begin{table}[H]
  \centering
  \caption{Public health KG trifecta.}
  \label{tab:public-health}
  \begin{tabular}{@{}lrrl@{}}
    \toprule
    \textbf{KG} & \textbf{Nodes} & \textbf{Edges} & \textbf{Source} \\
    \midrule
    Health Determinants & 285,635 & 285,628 & World Bank WDI + WHO \\
    Health Systems      & 19,661  & 19,428  & WHO SPAR + NHWA \\
    Surveillance        & 216,553 & 241,084 & WHO GHO \\
    \midrule
    \textbf{Total}      & \textbf{521,849} & \textbf{546,140} & \\
    \bottomrule
  \end{tabular}
\end{table}

The public health trifecta federates with the biomedical trifecta via \texttt{Country.iso\_code} bridge properties, enabling queries from ``What is India's PM2.5 exposure?'' to ``What IHR emergency capacity does India have?'' to ``What clinical trials are running in India?'' in a single Cypher session. \textbf{40 of 40 public health benchmark queries pass} with median latency 15\,ms; cross-KG joins average 191\,ms.

Together, the 6-KG federation comprises 8 open data sources, 8.6M+ registered nodes, and 30M+ edges---queryable from molecular biology to population health in a single OpenCypher query language.
...
\begin{itemize}[leftmargin=*]
  \item \textbf{Parse/plan overhead}: The PEG parser and query planner account for ${\sim}$98\% of Cypher query latency on small graphs. The plan cache eliminates planning overhead for repeated queries, but first-execution latency remains higher than pre-compiled systems (TigerGraph GSQL, Memgraph query modules).
  \item \textbf{MVCC}: Read-snapshot isolation foundation is implemented; full multi-version concurrency control is in progress for v0.8.0.
  \item \textbf{Snapshot export for stub-loaded graphs}: Graphs loaded via \texttt{create\_edge\_stub()} (used for the 1B-edge PubMed graph) currently require the edge store removal (v0.8.0) for portable snapshot export.
  \item \textbf{Cost-based planner maturity}: The cost model uses multiplicative cardinality estimates without histogram-based statistics, causing occasional timeouts on high-cardinality joins (1 of 100 biomedical queries).
  \item \textbf{Single-node architecture}: While a 3-node Raft cluster has been validated (426K nodes, all queries $<$1\,s), the billion-edge evaluation is single-node only. Distributed query execution over partitioned data is future work.
\end{itemize}
...
We have presented Samyama, a unified graph-vector database that integrates property graphs, vector search, metaheuristic optimization, and RDF support in a single Rust binary. The primary research contributions are: (1)~late materialization adapted from columnar databases to graph traversal, achieving 4.0--4.7$\times$ speedup; (2)~in-database metaheuristic optimization eliminating ETL overhead; (3)~agentic enrichment for autonomous graph expansion via LLMs; and (4)~\textbf{billion-edge knowledge graph federation} across 8 open data sources on commodity hardware for \$2.50 in compute.

Our evaluation demonstrates competitive ingestion (255K nodes/s), high OLTP throughput (115K QPS), 100\% LDBC Graphalytics validation, and---new in this version---74.3M nodes with 1.07B edges loaded and queried (96/100 biomedical queries, 40/40 public health queries). A 6-KG federation from molecular biology to population health is, to our knowledge, the first open-source demonstration of cross-domain knowledge graph federation at this scale.

\paragraph{Future work.} Key directions include: MVCC completion for full ACID transactions (v0.8.0); tiered hot/cold storage enabling billion-edge graphs on 32\,GB RAM; LDBC SF10 validation; histogram-based cardinality estimation; Picasso-style query plan space visualization; and expanding the benchmark to 200 queries across all 8 KGs.
