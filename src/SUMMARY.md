# Summary

[Preface](./preface.md)
- [About the Project & Author](./about.md)

# Part I: Knowledge Graphs & Queries
- [KG Catalog — 8 Knowledge Graphs, 75M Nodes](./kg_catalog.md)
- [Cross-KG Federation — Molecular Biology to Population Health](./kg_federation.md)
- [Ontology & Standards Catalog](./ontology_catalog.md)
- [Real-world Use Cases](./real_world_use_cases.md)

# Part II: Benchmarks & Results
- [Biomedical KG Benchmark (74M nodes, 1B edges, 100 queries)](./biomedical_benchmark.md)
- [Public Health KG Benchmark (305K nodes, 40 queries)](./public_health_benchmark.md)
- [Performance & Benchmarks (ingestion, OLTP, GPU, LDBC)](./performance_benchmarks.md)

# Part III: The Engine
- [The Query Engine](./query_engine.md)
- [Query Optimization (Explain)](./query_optimization.md)
- [Persistence at Scale](./persistence_at_scale.md)
- [Managing State (MVCC & Memory)](./managing_state.md)
- [Analytical Power (CSR & Algorithms)](./analytical_power.md)
- [In-Database Optimization (Metaheuristics)](./in_database_optimization.md)
- [Constrained Multi-Objective Optimization](./constrained_optimization.md)
- [Technology Choices (The "Why")](./technology_choices.md)

# Part IV: Distributed & AI
- [Distributed Consensus (Raft)](./distributed_consensus.md)
- [Production-Grade High Availability](./advanced_raft.md)
- [AI & Vector Search](./ai_vector_search.md)
- [Agentic Enrichment](./agentic_enrichment.md)
- [RDF & SPARQL Support](./rdf_sparql.md)
- [Predictive Power (GNNs) — Roadmap](./predictive_power.md)

# Part V: Enterprise & Operations
- [Enterprise Edition Overview](./samyama_enterprise.md)
- [Backup & Disaster Recovery](./backup_recovery.md)
- [Administrative Protocol](./admin_protocol.md)
- [Observability & Multi-tenancy](./observability_multi_tenancy.md)
- [SDKs, CLI & API](./sdk_cli_api.md)
- [Ecosystem Architecture](./ecosystem_architecture.md)
- [The Future of Graph DBs](./future_roadmap.md)

# Appendices
- [Frequently Asked Questions](./faq.md)
- [BiomedQA Benchmark FAQ](./biomedqa_faq.md)
- [Glossary](./glossary.md)

# Research & Publications
- [Publication Tracker](./publication_tracker.md)
## arxiv Preprints
- [Paper 1: Samyama Unified Graph-Vector Database (arxiv:2603.08036)](./research_paper_summary.md)
- [Paper 4: Open Biomedical Knowledge Graphs (arxiv:2603.15080)](./biomedical_kg_paper.md)
- [Paper 7: Billion-Edge KG Federation (in preparation)](./paper7_billion_edge_federation.md)
## Conference Submissions
- [Paper 5: Federated Biomedical KGs — GRADES-NDA @ SIGMOD 2026](./paper5_grades_nda.md)
- [Paper 6: MCP Tools vs Text-to-Cypher — aiDM @ SIGMOD 2026](./paper6_aidm.md)
- [Paper 3: Industrial KG (AssetOpsBench) — NeurIPS E&D 2026](./industrial_kg_paper.md)
- [VLDB 2027 Systems Paper (in preparation)](./vldb_2027.md)
## Reference
- [Index of Implemented Papers](./implemented_papers.md)

# Presentations
- [Samyama Overview Slides (HTML)](./slides.html)
